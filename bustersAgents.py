...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # initialize the closest ghost information for comparisons
        closestGhostDist = float("inf")
        closestPosition = None
        # for each living ghost distribution
        for ghostDistributions in livingGhostPositionDistributions:
            prob = 0
            position = None
            # go through each finding the most likely position, with its probability
            for p in ghostDistributions:
                if ghostDistributions[p] > prob:
                    prob = ghostDistributions[p]
                    position = p
            # then find the maze distance there and store it if it is the closest
            # found so far
            distance = self.distancer.getDistance(pacmanPosition, position)
            if distance < closestGhostDist:
                closestGhostDist = distance
                closestPosition = position
        # for each possible action, calculate the successor position and return
        # the action that results in the shortest distance
        bestAction = None
        shortestDistance = closestGhostDist
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            if self.distancer.getDistance(successorPosition, closestPosition) < shortestDistance:
                shortestDistance = self.distancer.getDistance(successorPosition, closestPosition)
                bestAction = action
        return bestAction
